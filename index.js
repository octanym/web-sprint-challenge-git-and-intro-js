...
function getArtistByIndex(arr, target) {
  return `the artist at index ${arr[target].id} is ${arr[target].name}`;
}
...
function listOfNames(arr) {
  return arr.map((artist) => artist.name);
}
...
function removeArtist(arr, target) {
  let modArr = arr.slice();
  modArr.splice(target, 1);
  return modArr;
}
...
function artistInfo(arr, name) {
  const chosenArtist = arr.filter((artist) => artist.name === name);
  return chosenArtist[0].bio;
}
...
function artistByCountry(arr, nationality) {
  return arr
    ...
    .map((compatriots) => compatriots.name);
}
